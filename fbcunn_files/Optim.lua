...
-- from fblualib/fb/util/data.lua , 
-- copied here because fblualib is not rockspec ready yet.
...
function Optim:save()
  return { optim_state = self.modulesToOptState }
end
...
local function get_device_for_module(mod)
  ...
  -- _may still be zero if none are allocated.
  return dev_id
end
...
function Optim:optimize(optimMethod, inputs, targets, criterion)
  ...
  -- We'll set these in the loop that iterates over each module. 
  -- Get them out here to be captured.
  local curGrad
  local curParam
